...
class VAF:
    ...
    def tcs(self,lpf=0, opf=1):
        ...
        #Mean data
        self.ldata[0][1] = S.moment0_env[0]
        self.ldata[0][2] = S.moment0_env[2]
        self.ldata[0][3] = S.moment0_env[4]
        self.ldata[0][4] = S.moment0_rms[0]
        self.ldata[0][5] = S.moment0_rms[2]
        ...
        for i in range(1,len(self.M)):
            ...
            #Mean data
            self.ldata[i][1] = S.moment0_env[0]
            self.ldata[i][2] = S.moment0_env[2]
            self.ldata[i][3] = S.moment0_env[4]
            self.ldata[i][4] = S.moment0_rms[0]
            self.ldata[i][5] = S.moment0_rms[2]
            self.ldata[i][6] = S.ref_phis
            self.ldata[i][7] = S.ref_IonEk
            
        #output data for plotting
        if opf: numpy.savetxt('ldata.txt',self.ldata)

        if not lpf: return S

        
    def tcs_seg(self,start,end,SD=None, allcs=0):
        """
        ...
        (S, rdata, adata) = tcs_seg(start_index, end_index, SD=beam_data, allcs=1)
        adata[k][i] contains:
        ...
        if allcs: adata = [[[0.0]*6 for i in range(fin)] for j in range(len(S.IonZ))]

        # store initial beam data
        rdata[0][0] = S.pos
        rdata[0][1] = S.moment0_env[0]
        rdata[0][2] = S.moment0_env[2]
        rdata[0][3] = S.moment0_env[4]
        rdata[0][4] = S.moment0_rms[0]
        rdata[0][5] = S.moment0_rms[2]
        rdata[0][6] = S.ref_phis - phis_ini
        rdata[0][7] = S.ref_IonEk

        if allcs:
            for k in range(len(S.IonZ)):
                adata[k][0][0] = S.pos
                adata[k][0][1] = S.moment0[0,k]
                adata[k][0][2] = S.moment0[2,k]
                adata[k][0][3] = S.moment0[4,k]
                adata[k][0][4] = numpy.sqrt(S.moment1[0,0,k])
                adata[k][0][5] = numpy.sqrt(S.moment1[2,2,k])
        ...
        for (j,i) in enumerate(range(start,end)):
            ...
            rdata[j+1][0] = S.pos
            rdata[j+1][1] = S.moment0_env[0]
            rdata[j+1][2] = S.moment0_env[2]
            rdata[j+1][3] = S.moment0_env[4]
            rdata[j+1][4] = S.moment0_rms[0]
            rdata[j+1][5] = S.moment0_rms[2]
            rdata[j+1][6] = S.ref_phis - phis_ini
            rdata[j+1][7] = S.ref_IonEk

            if allcs:
                for k in range(len(S.IonZ)):
                    adata[k][j+1][0] = S.pos
                    adata[k][j+1][1] = S.moment0[0,k]
                    adata[k][j+1][2] = S.moment0[2,k]
                    adata[k][j+1][3] = S.moment0[4,k]
                    adata[k][j+1][4] = numpy.sqrt(S.moment1[0,0,k])
                    adata[k][j+1][5] = numpy.sqrt(S.moment1[2,2,k])

        
        if allcs: return (S,rdata,adata)
        else : return (S,rdata)        
